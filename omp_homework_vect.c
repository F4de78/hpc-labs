...
// size of input array
    ...
    double* xr = (double*) _mm_malloc (N *sizeof(double), 64);
    double* xi = (double*) _mm_malloc (N *sizeof(double), 64);
    fillInput(xr,xi,N);

    double* xr_check = (double*) _mm_malloc (N *sizeof(double), 64);
    double* xi_check = (double*) _mm_malloc (N *sizeof(double), 64);
    setOutputZero(xr_check,xi_check,N);

    double* Xr_o = (double*) _mm_malloc (N *sizeof(double), 64);
    double* Xi_o = (double*) _mm_malloc (N *sizeof(double), 64);
...
int DFT(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o, int N){
  ...
  for (k=0 ; k<N ; k++)
  {
      double tmp_r = 0; // si puo sostituire con pragma omp simd
      double tmp_i = 0;
      __assume_aligned(xr, 64);
      __assume_aligned(xi, 64);
      for (n=0 ; n<N ; n++)  {
        // Real part of X[k]
          tmp_r += xr[n] * cos(n * k * PI2 / N) + idft*xi[n]*sin(n * k * PI2 / N);
          // Imaginary part of X[k]
          tmp_i += -idft*xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
      }
      Xr_o[k] = tmp_r;
      Xi_o[k] = tmp_i;
